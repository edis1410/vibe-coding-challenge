...
import { validatePercentageSplit, validateAmountSplit, calculatePercentageSplit } from '@/lib/split-validation'
import { use } from 'react'

export default function NewExpense({ params }: { params: Promise<{ groupId: string }> }) {
      ...
      try {
        ...
        if (groupMembers.length > 0) {
          ...
          memberIds.forEach(id => {
            initialPercentages[id] = Number((100 / memberIds.length).toFixed(2))
            initialAmounts[id] = 0
          })
          setPercentageSplits(initialPercentages)
          setAmountSplits(initialAmounts)
        }
      } catch (err: unknown) {
    ...
    <div className="min-h-screen bg-gray-950">
      ...
      <main className="max-w-2xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        ...
        <div className="bg-gray-900 border border-gray-800 rounded-2xl shadow-xl p-8">
          ...
          <p className="text-gray-400 mb-8">Record a new expense for the group</p>
                  ...
                  <div className="space-y-2">
                    {members.map((member: any) => {
                      ...
                      return (
                        <div key={member.user_id} className="flex items-center gap-3 p-3 bg-gray-800 border border-gray-700 rounded-lg">
                          <span className="text-sm font-medium text-gray-200 flex-1">{displayName}</span>
                          <span className="text-sm text-gray-400">{getCurrencySymbol(currency)}</span>
                          <input
                            type="number"
                            step="0.01"
                            min="0"
